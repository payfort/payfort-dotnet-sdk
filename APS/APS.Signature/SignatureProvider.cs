...
namespace APS.Signature
{
    ...
    {
        public string GetSignature<T>(T input, string shaPhrase, ShaType shaType)
        {
        ...
        private string PrepareSignature<T>(T input, string shaPhrase)
        {
            ...
            foreach (var prop in orderedProperties)
            {
                ...
                var propertyValue = prop.GetValue(input, null);
                    ...
                    prop.GetJsonPropertyIgnoreSignatureValidation())
                {
                    continue;
                }
        ...
        private static void BuildSignatureForComplexItem(PropertyInfo propertyInfo, object propertyValue,
        ...
        private static void BuildSignatureForListItem(string propertyName, object propertyValue,
            StringBuilder signatureBuilder)
        {
        ...
        private static string GetCsvFormattedProperties(object input)
        {
            ...
            foreach (var prop in properties)
            {
                ...
                if (!prop.PropertyType.IsSimpleObject())
                {
                    if (prop.PropertyType.IsListObject())
                    {
                        BuildSignatureForListItem(jsonPropertyName, propertyValue, csvFormattedPropertiesBuilder);
                        csvFormattedPropertiesBuilder.Append(", ");
                    }
                    else
                    {
                        csvFormattedPropertiesBuilder.Append($"{jsonPropertyName}={GetCsvFormattedProperties(propertyValue).Trim()}, ");
                    }
                    continue;
                }
